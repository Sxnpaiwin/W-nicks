         ...
         new CommandAPICommand("list")
            ...
            .executes((sender, args) -> {
               if (plugin.getFakeRankManager() == null) {
                  sender.sendMessage(MiniMsg.deserialize("<red>LuckPerms is not installed. Fake rank support is disabled."));
                  return;
               }
               ...
               sender.sendMessage(MiniMsg.deserialize("<gold><bold>Available ranks" + (onlyAssignable ? " (assignable)" : "") + ":"));
               ...
               sender.sendMessage(MiniMsg.deserialize("<gray>Use <white>/nickrank set <rank></white> to apply a rank to yourself."));
            }, new ExecutorType[0])
      );
      ...
      this.withSubcommand(
         new CommandAPICommand("set")
            ...
            .executes((sender, args) -> {
               ...
               if (resolvedId == null) {
                  sender.sendMessage(MiniMsg.deserialize("<red>Unknown rank: " + rankInput));
                  sender.sendMessage(MiniMsg.deserialize("<gray>Use <white>/nickrank list</white> to see available ranks."));
                  return;
               }
               ...
               for (Player target : targets) {
                  if (!sender.hasPermission("wnick.commands.nickrank.others") && target != sender) {
                     sender.sendMessage(MiniMsg.deserialize("<red>You do not have permission to change other players' fake rank."));
                     return;
                  }
                  applyFakeRank(plugin, target, resolvedId);
                  String preview = rank.prefix() + (target.getName()) + rank.suffix();
                  sender.sendMessage(
                     MiniMsg.deserialize("<green>Set fake rank of </green>")
                        .append(LegacyComponentSerializer.legacySection().deserialize(target.getName()))
                        .append(MiniMsg.deserialize(" <green>to </green>"))
                        .append(MiniMsg.deserialize("<white>" + rank.id() + "</white>"))
                  );
                  if (!rank.prefix().isEmpty() || !rank.suffix().isEmpty()) {
                     sender.sendMessage(
                        MiniMsg.deserialize("<gray>Preview: ")
                           .append(LegacyComponentSerializer.legacyAmpersand().deserialize(preview))
                     );
                  }
                  if (!plugin.hasNick(target)) {
                     sender.sendMessage(MiniMsg.deserialize("<yellow>Note: " + target.getName() + " is not currently nicked. The rank will apply once they /nick."));
                  }
               }
            }, new ExecutorType[0])
      );
      ...
      this.withSubcommand(
         new CommandAPICommand("clear")
            ...
            .executes((sender, args) -> {
               ...
               for (Player target : targets) {
                  if (!sender.hasPermission("wnick.commands.nickrank.others") && target != sender) {
                     sender.sendMessage(MiniMsg.deserialize("<red>You do not have permission to clear other players' fake rank."));
                     return;
                  }
                  plugin.clearFakeRank(target);
                  sender.sendMessage(
                     MiniMsg.deserialize("<green>Cleared fake rank of </green>")
                        .append(LegacyComponentSerializer.legacySection().deserialize(target.getName()))
                  );
               }
            }, new ExecutorType[0])
      ...
      this.withSubcommand(
         new CommandAPICommand("random")
            ...
            .executes((sender, args) -> {
               ...
               if (random == null) {
                  sender.sendMessage(MiniMsg.deserialize("<red>No assignable ranks found. Tag a LuckPerms group with the permission <gray>wnick.rank.assignable</gray> to make it assignable."));
                  return;
               }
               @SuppressWarnings("unchecked")
               List<Player> targets = (List<Player>) args.get("targets");
               ...
               for (Player target : targets) {
                  ...
                  sender.sendMessage(
                     MiniMsg.deserialize("<green>Assigned random fake rank </green>")
                        .append(MiniMsg.deserialize("<white>" + random.id() + "</white>"))
                        .append(MiniMsg.deserialize("<green> to </green>"))
                        .append(LegacyComponentSerializer.legacySection().deserialize(target.getName()))
                  );
               }
            ...
            .executes((sender, args) -> {
               ...
               if (args.get("target") instanceof Player p) {
                  if (!sender.hasPermission("wnick.commands.nickrank.others") && p != sender) {
                     sender.sendMessage(MiniMsg.deserialize("<red>You do not have permission to view other players' fake rank."));
                     return;
                  }
                  target = p;
               ...
               if (rankId == null) {
                  sender.sendMessage(
                     MiniMsg.deserialize("<gray>" + target.getName() + " has no fake rank set.</gray>")
                  );
                  return;
               }
               if (plugin.getFakeRankManager() == null) {
                  sender.sendMessage(
                     MiniMsg.deserialize("<yellow>" + target.getName() + " has fake rank id <white>" + rankId + "</white>, but LuckPerms is no longer available to resolve its details.</yellow>")
                  );
                  return;
               }
               ...
               if (rank == null) {
                  sender.sendMessage(
                     MiniMsg.deserialize("<yellow>" + target.getName() + " has fake rank id <white>" + rankId + "</white>, but that group no longer exists in LuckPerms.</yellow>")
                  );
                  return;
               }
               Component line = Component.empty()
