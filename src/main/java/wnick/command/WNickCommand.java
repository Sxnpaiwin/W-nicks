      ...
      this.withSubcommand(
         new CommandAPICommand("reload")
            ...
            .executes((sender, args) -> {
               ...
               sender.sendMessage(plugin.prefixedMessage("<gray><italic>Config reloaded."));
            }, new ExecutorType[0])
      );
   }

   private static void sendHelp(CommandSender sender, NameTagPlugin plugin) {
      sender.sendMessage(MiniMsg.deserialize("<gold><bold>W-Nick</bold> <gray>by Joehe</gray>"));
      sender.sendMessage(MiniMsg.deserialize("<dark_gray>━━━━━━━━━━━━━━━━━━━━━━━━━━━</dark_gray>"));
      sender.sendMessage(MiniMsg.deserialize("<yellow>/nick <reset|as|with_name|with_skin|from_url> [name]"));
      sender.sendMessage(MiniMsg.deserialize("<gray>   Set, reset, or change your nick. Add <white>-r <rank></white> to spoof a LuckPerms rank.</gray>"));
      sender.sendMessage(MiniMsg.deserialize("<yellow>/randomnick [selector] [-r <rank>] [--skip]"));
      sender.sendMessage(MiniMsg.deserialize("<gray>   Generate a random nick (optionally with a specific rank).</gray>"));
      sender.sendMessage(MiniMsg.deserialize("<yellow>/nickrank <list|set|clear|random|current>"));
      sender.sendMessage(MiniMsg.deserialize("<gray>   Manage the LuckPerms fake rank independently of your nick.</gray>"));
      sender.sendMessage(MiniMsg.deserialize("<yellow>/realname <nick>"));
      sender.sendMessage(MiniMsg.deserialize("<gray>   Look up who's behind a nick.</gray>"));
      sender.sendMessage(MiniMsg.deserialize("<yellow>/wnick guide"));
      sender.sendMessage(MiniMsg.deserialize("<gray>   Open this help as an interactive in-game dialog (Paper 1.21.8+).</gray>"));
      sender.sendMessage(MiniMsg.deserialize("<yellow>/wnick info [player]"));
      sender.sendMessage(MiniMsg.deserialize("<gray>   Show all known nick state for yourself or someone else.</gray>"));
      sender.sendMessage(MiniMsg.deserialize("<yellow>/wnick version / reload"));
      sender.sendMessage(MiniMsg.deserialize("<gray>   Version info and config reload.</gray>"));
      sender.sendMessage(MiniMsg.deserialize("<dark_gray>━━━━━━━━━━━━━━━━━━━━━━━━━━━</dark_gray>"));
      if (plugin.getFakeRankManager() == null) {
         sender.sendMessage(MiniMsg.deserialize("<red>LuckPerms is not installed. Rank-spoofing features are disabled.</red>"));
      } else {
