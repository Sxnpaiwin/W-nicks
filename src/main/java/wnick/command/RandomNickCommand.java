         ...
         new Argument[]{
            new GreedyStringArgument("args")
               ...
               .replaceSuggestions((info, builder) -> {
                  ...
                  boolean hasWordFlag = Flags.hasWordFlag(info.currentArg(), "skip");
                  String stripped = Flags.sanitizeInput(current, Set.of(), Set.of("skip")).trim();
                  if (stripped.isEmpty()) {
      ...
      this.executes(
         (sender, args) -> {
            ...
            if (rankOverride != null) {
               if (plugin.getFakeRankManager() == null) {
                  sender.sendMessage(MiniMsg.deserialize("<red>LuckPerms is not installed. The -r flag is unavailable."));
                  return;
               }
               String resolvedId = plugin.getFakeRankManager().resolveRankIdCaseInsensitive(rankOverride);
               if (resolvedId == null) {
                  sender.sendMessage(MiniMsg.deserialize("<red>Unknown rank: " + rankOverride));
            ...
            boolean skip = Flags.hasWordFlag(rawArgsNoRank, "skip");
            String selectorStr = rawArgsNoRank != null ? Flags.sanitizeInput(rawArgsNoRank, Set.of(), Set.of("skip")).trim() : "";
            if (selectorStr.isEmpty()) {
               if (sender instanceof Player player) {
                  if (!skip || !plugin.hasNick(player)) {
                     sender.sendMessage(MiniMsg.deserialize("<yellow>Generating random nickname" + (rankOverride != null ? " with rank " + rankOverride : "") + "...</yellow>"));
                     if (rankOverride != null) {
                  ...
                  sender.sendMessage(MiniMsg.deserialize("<red>This command can only be executed by players when no target is specified."));
               }
               ...
               try {
                  ...
                  sender.sendMessage(MiniMsg.deserialize("<red>Invalid selector: " + selectorStr));
                  return;
               }

               if (targets.isEmpty()) {
                  sender.sendMessage(MiniMsg.deserialize("<red>No players matched."));
               } else {
                  int skipped = 0;
                  int queued = 0;
                  ...
                  if (skipped > 0) {
                     sender.sendMessage(
                        MiniMsg.deserialize("<gray>Skipped " + skipped + " already-nicked player" + (skipped > 1 ? "s" : "") + ".</gray>")
                     );
                  }
               }
            }
