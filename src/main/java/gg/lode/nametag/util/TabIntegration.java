   ...
   private void updateTabPlayer(Object tabPlayer) {
      ...
      try {
         ...
         if (nicked && plugin.getFakeRankManager() != null) {
            gg.lode.nametagapi.api.NickPlayer nickData = plugin.getPlayerCache().get(uuid);
            if (nickData != null && nickData.getFakeRankId() != null) {
               gg.lode.nametag.util.FakeRankManager.FakeRank rank = plugin.getFakeRankManager().getRank(nickData.getFakeRankId());
               if (rank != null) {
